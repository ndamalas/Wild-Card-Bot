...
async def getStock(client, message):
	...
	#use try/except to catch bad ticker names
	try:
		...
		if len(link) > 1:
			...
			await message.channel.send("Error occurred when retrieving stock. Perhaps the ticker does not exist or is misspelled?")